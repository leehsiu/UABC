...
class UABCNet(nn.Module):
	def __init__(self, n_iter=8, h_nc=64, in_nc=4, out_nc=3, nc=[64, 128, 256, 512], nb=2, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose'):
		...
		self.proj = ResUNet(in_nc=in_nc, out_nc=out_nc, nc=nc, nb=nb, act_mode=act_mode, downsample_mode=downsample_mode, upsample_mode=upsample_mode)
	...
	def assemble_patches(self,z,patch_num,patch_size):
		...
		x_01 = torch.cat(torch.split(z[::patch_num[0]][...,0:w_pad,h_pad:h_pad+patch_size[1]],1),dim=3)
		x_02 = z[(patch_num[1]-1)*patch_num[0]:(patch_num[1]-1)*patch_num[0]+1][...,:w_pad,h_pad+patch_size[1]:]
		x_0 = torch.cat([x_00,x_01,x_02],dim=3)

		x_10 = torch.cat(torch.split(z[:patch_num[0]][...,w_pad:w_pad+patch_size[0],:h_pad],1),dim=2)
		x_11 = torch.cat(torch.chunk(z[...,w_pad:w_pad+patch_size[0],h_pad:h_pad+patch_size[1]],patch_num[1],dim=0),dim=3)
		x_11 = torch.cat(torch.chunk(x_11,patch_num[0],dim=0),dim=2)
		x_12 = torch.cat(torch.split(z[patch_num[0]*(patch_num[1]-1):patch_num[0]*patch_num[1]][...,w_pad:w_pad+patch_size[0],h_pad+patch_size[1]:],1),dim=2)
		x_1 = torch.cat([x_10,x_11,x_12],dim=3)

		x_20 = z[patch_num[0]-1:patch_num[0]][...,w_pad+patch_size[0]:,:h_pad]
		x_21 = torch.cat(torch.split(z[patch_num[0]-1::patch_num[0]][...,:w_pad,h_pad:h_pad+patch_size[1]],1),dim=3)
		x_22 = z[patch_num[0]*patch_num[1]-1:patch_num[0]*patch_num[1]][...,w_pad+patch_size[0]:,h_pad+patch_size[1]:]
	...
	def forward_patchwise(self,y,k,ab,patch_num=[2,2],patch_size=[128,128]):
		...
		for h_ in range(patch_num[1]):
			for w_ in range(patch_num[0]):
				...
				Fk = psf2otf(k[w_+h_*patch_num[0]].unsqueeze(0),(patch_size[0]+w_pad*2,patch_size[1]+h_pad*2))
		...
		for i in range(self.n-1):
			...
			for idx in range(patch_num[0]*patch_num[1]):
				...
				z_p = self.deconv(z[idx:idx+1],FkCFy_all[idx],FkCFk_all[idx],mu_p)
				z_temp.append(z_p)
